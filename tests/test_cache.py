...
class TestLimitedSizeDict:
    ...
    def test_init(self) -> None:
        ...
        cache = LimitedSizeDict(max_size=CACHE_SIZE_5)
        assert cache.max_size == CACHE_SIZE_5
        assert len(cache) == 0

    def test_add_items_within_limit(self) -> None:
        """Test adding items within the size limit."""
        cache = LimitedSizeDict(max_size=CACHE_SIZE_3)
        cache["key1"] = "value1"
        cache["key2"] = "value2"
        cache["key3"] = "value3"

        assert len(cache) == CACHE_SIZE_3
    ...
    def test_max_size_one(self) -> None:
        ...
        cache = LimitedSizeDict(max_size=CACHE_SIZE_1)
        cache["key1"] = "value1"
        assert len(cache) == CACHE_SIZE_1
        assert cache["key1"] == "value1"
    ...
    def test_multiple_evictions(self) -> None:
        ...
        cache = LimitedSizeDict(max_size=CACHE_SIZE_2)
        cache["key1"] = "value1"
        cache["key2"] = "value2"
        cache["key3"] = "value3"  # Evicts key1
        cache["key4"] = "value4"  # Evicts key2

        assert len(cache) == CACHE_SIZE_2
        assert "key1" not in cache
        assert "key2" not in cache
        assert cache["key3"] == "value3"
        assert cache["key4"] == "value4"
